...
const vacancies = [
  ...
  { title: "Kettle Club Helper", commitment: "Weekly", involved: ["Helping prepare breakfast and refreshments", "Setting up and clearing the social space", "Welcoming residents and starting conversations", "Compassionate and friendly manner needed"], requirements: ["No experience needed", "Warm and approachable personality", "Reliable availability"] },
];
const positions = ["Volunteer Driver", "Kettle Club Helper", "Food Van Assistant", "Admin Support", "Fundraising", "General Volunteer", "Other"];
            ...
            {vacancies.map((v, i) => (
              <AnimatedSection key={v.title} delay={i * 0.1}>
                <div className="card-wapm p-8 h-full">
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1 mb-4">{v.involved.map(item => <li key={item}>• {item}</li>)}</ul>
                  <h4 className="font-medium text-foreground text-sm mb-2">Requirements:</h4>
                  <ul className="text-sm text-muted-foreground space-y-1 mb-6">{v.requirements.map(item => <li key={item}>• {item}</li>)}</ul>
